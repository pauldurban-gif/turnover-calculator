...
const C = {
  ...
  amberLight:   "#fef3c7",
  amberBorder:  "#fcd34d",
  cardGrad:     "linear-gradient(160deg, #ffffff 0%, #f4f5f7 100%)",
  cardGradAlt:  "linear-gradient(160deg, #f8f9fb 0%, #eceef2 100%)",
  cardGradTeal: "linear-gradient(160deg, #edfafa 0%, #d6f3f3 100%)",
  cardGradNavy: "linear-gradient(160deg, #001e46 0%, #002d66 100%)",
  text:         "#001e46",
  textMid:      "#2a3f5f",
  textMuted:    "#5a7090",
  textFaint:    "#8a9ab0",
  shadowSm:     "0 1px 2px rgba(0,30,70,0.06), 0 3px 8px rgba(0,30,70,0.07)",
  shadow:       "0 2px 4px rgba(0,30,70,0.07), 0 6px 20px rgba(0,30,70,0.09)",
  shadowLg:     "0 4px 8px rgba(0,30,70,0.08), 0 16px 40px rgba(0,30,70,0.13)",
  shadowTeal:   "0 2px 4px rgba(0,173,176,0.15), 0 6px 20px rgba(0,173,176,0.18)",
  shadowNavy:   "0 4px 8px rgba(0,30,70,0.25), 0 12px 32px rgba(0,30,70,0.30)",
  border:       "#dde2ea",
  borderLight:  "#eaedf2",
};
...
const ORG_TYPES = [
  { id: "human_services", label: "Human Services",   icon: "🤝", sectorMultiplier: 1.20,
    ...
    filamentBridge: "Filament helps Human Services organizations work through exactly this — compensation benchmarking, stay interview programs, and supervisor development are core parts of what we do.",
  },
  { id: "health", label: "Health & Wellness", icon: "🏥", sectorMultiplier: 1.30,
    ...
    filamentBridge: "Filament works with Health & Wellness nonprofits on exactly these challenges — market compensation analysis, compliance workflow reviews, and benefits structuring that competes with larger health systems.",
  },
  { id: "education", label: "Education", icon: "📚", sectorMultiplier: 1.15,
    ...
    filamentBridge: "Filament helps Education nonprofits build the internal structures that keep talented staff — career ladders with real salary bands, scheduling models that compete with schools, and handoff systems that protect program continuity.",
  },
  { id: "arts", label: "Arts & Culture", icon: "🎨", sectorMultiplier: 0.95,
    ...
    filamentBridge: "Filament helps Arts & Culture organizations untangle the specific pressures that drive turnover here — role and compensation audits, board-staff boundary clarity, and communication strategies that build stability even in uncertain funding environments.",
  },
  { id: "environment", label: "Environment", icon: "🌿", sectorMultiplier: 0.90,
    ...
    filamentBridge: "Filament helps Environmental organizations build the HR infrastructure that sustains mission-driven staff for the long haul — workload design, professional development structures, and compensation strategies that close the gap with government and foundation roles.",
  },
  { id: "advocacy", label: "Advocacy / Policy", icon: "📣", sectorMultiplier: 1.10,
    ...
    filamentBridge: "Filament helps Advocacy organizations protect against exactly this kind of institutional knowledge risk — knowledge documentation systems, succession planning, and proactive retention strategies timed to your policy calendar.",
  },
  { id: "faith", label: "Faith-Based", icon: "✦", sectorMultiplier: 0.85,
    ...
    filamentBridge: "Filament works with Faith-Based organizations on the HR practices that protect long-term health — compensation benchmarking against the broader nonprofit market, succession planning before a transition happens, and knowledge transfer systems that outlast any individual.",
  },
  { id: "other", label: "Other Nonprofit", icon: "◎", sectorMultiplier: 1.00,
    ...
    filamentBridge: "Filament helps nonprofits work through all of these — compensation benchmarking, career pathway design, and manager effectiveness programs are core to what we do with organizations like yours.",
  },
];
...
function ResultScreen({ orgType, size, salary, turnover, onReset }) {
  ...
  const benchmark   = orgType.benchmarkRate;
  const userRate    = turnover.rate;
  const benchmarkLabel = userRate < benchmark - 0.05 ? "below average"
                       : userRate > benchmark + 0.05 ? "above average"
                       : "near the average";
  const benchmarkColor = userRate <= benchmark ? C.tealDark : C.amber;
  const reducedRateDisplay = Math.round(reducedRate * 100);
  const currentRateDisplay = Math.round(userRate * 100);
  ...
  const handleCopy = () => {
    const text = [
      ...
      ``,
      `SECTOR BENCHMARK:`,
      `  Your rate: ${currentRateDisplay}%   |   ${orgType.label} average: ${Math.round(benchmark * 100)}%`,
      `  Your turnover is ${benchmarkLabel} for ${orgType.label} organizations.`,
      ``,
      `RETENTION ROI:`,
      roiMode === "projection"
  ...
  return (
    <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(16px)", transition: "all 0.5s ease" }}>
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: C.shadowSm, padding: "20px 24px", marginBottom: "16px" }}>
        <SectionLabel text="How you compare" />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "10px", gap: "12px" }}>
          <div>
            <p style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMid, lineHeight: 1.6 }}>
              The average turnover rate for <strong>{orgType.label}</strong> nonprofits is <strong style={{ color: C.navy }}>{Math.round(benchmark * 100)}%</strong>. Your rate of <strong style={{ color: benchmarkColor }}>{currentRateDisplay}%</strong> is <strong style={{ color: benchmarkColor }}>{benchmarkLabel}</strong> for your sector.
            </p>
          </div>
        </div>
        ...
        <div style={{ marginTop: "14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint }}>0%</span>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint }}>50%+</span>
          </div>
          <div style={{ position: "relative", height: "10px", background: C.grayLight, borderRadius: "5px", overflow: "visible" }}>
            {/* Track fill up to user rate */}
            <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: `${Math.min(userRate / 0.55, 1) * 100}%`, background: userRate > benchmark ? C.amber : C.teal, borderRadius: "5px", transition: "width 1s ease 0.3s" }} />
            {/* Benchmark marker */}
            <div style={{ position: "absolute", top: "-4px", left: `${Math.min(benchmark / 0.55, 1) * 100}%`, transform: "translateX(-50%)", width: "3px", height: "18px", background: C.navy, borderRadius: "2px" }} />
            <div style={{ position: "absolute", top: "-22px", left: `${Math.min(benchmark / 0.55, 1) * 100}%`, transform: "translateX(-50%)", fontFamily: "Inter, sans-serif", fontSize: "10px", fontWeight: 700, color: C.navy, whiteSpace: "nowrap" }}>
              Avg {Math.round(benchmark * 100)}%
            </div>
            {/* User marker */}
            <div style={{ position: "absolute", top: "-4px", left: `${Math.min(userRate / 0.55, 1) * 100}%`, transform: "translateX(-50%)", width: "14px", height: "18px", background: benchmarkColor, borderRadius: "3px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: "8px", fontWeight: 700, color: "#fff" }}>You</span>
            </div>
          </div>
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint, marginTop: "16px", lineHeight: 1.5 }}>
            Sector benchmark source: {orgType.source}
          </p>
        </div>
      </div>
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: C.shadow, marginBottom: "16px", overflow: "hidden" }}>
        <div style={{ padding: "20px 24px 18px", borderBottom: `1px solid ${C.borderLight}`, background: C.cardGradAlt }}>
          ...
          <p style={{ fontFamily: "'Lora', serif", fontSize: "18px", fontWeight: 700, color: C.navy, lineHeight: 1.3 }}>
            The factors most likely driving turnover at your organization
          </p>
        </div>
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "14px", boxShadow: C.shadow, padding: "32px", textAlign: "center", marginBottom: "20px" }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "14px", color: C.textMuted, lineHeight: 1.8, maxWidth: "420px", margin: "0 auto" }}>
          You now have the number, the risk factors, and the first steps. The next move is a conversation — and it doesn't have to be complicated. Filament works alongside {orgType.label} organizations to turn this diagnosis into a plan.
        </p>
      </div>
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#ffffff", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 20px 80px" }}>
        ...
        <div style={{ width: "100%", maxWidth: "660px", padding: "48px 0 0", marginBottom: "8px", textAlign: "center" }}>
          ...
          <div style={{ width: "48px", height: "3px", background: `linear-gradient(90deg, ${C.navy}, ${C.teal})`, borderRadius: "2px", margin: "20px auto 0" }} />
        </div>
