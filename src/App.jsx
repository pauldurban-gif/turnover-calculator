...
function calcCosts(orgType, size, salary, turnover) {
  ...
  // Compute total from unrounded values — round only once at the end
  const totalRaw             = departures * salary.midpoint * effectiveMultiplier;
  const total                = Math.round(totalRaw / 1000) * 1000;

  // Derive all display values from total — never round independently
  ...
  return { total, perMonth, departures, costPerDeparture, employees, baseMultiplier,
           sectorMultiplier, effectiveMultiplier, visibleTotal, invisibleTotal,
...
function AnimatedNumber({ value, duration = 1600 }) {
  ...
  useEffect(() => {
    ...
    const animate = ts => {
      ...
      setDisplay(Math.round((1 - Math.pow(2, -10 * p)) * rounded));
...
function ResultScreen({ orgType, size, salary, turnover, onReset }) {
  ...
  const { total, perMonth, departures, costPerDeparture, visibleTotal, invisibleTotal,
...
// ─── Steps ────────────────────────────────────────────────────────────────────
const STEPS = [
  { id: "orgType",  question: "What type of nonprofit do you run?",       sub: "Each sector has distinct turnover dynamics, and this shapes how we calibrate the estimate." },
