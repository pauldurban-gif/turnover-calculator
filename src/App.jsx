...
const ORG_TYPES = [
  { id: "human_services", label: "Human Services",   icon: "🤝", sectorMultiplier: 1.20,
    ...
    risks: [
      ...
      { factor: "Compensation compression",       detail: "Entry and mid-level salaries often haven't kept pace with inflation, narrowing the gap between new hires and experienced staff and reducing incentive to stay." },
  ...
  { id: "arts", label: "Arts & Culture", icon: "🎨", sectorMultiplier: 0.95,
    ...
    firstSteps: [
      ...
      "Communicate funding timelines and position security proactively, since uncertainty is more damaging than bad news.",
  ...
  { id: "environment", label: "Environment", icon: "🌿", sectorMultiplier: 0.90,
    ...
    benchmarkRate: 0.18,
    risks: [
      { factor: "Urgency fatigue",                detail: "Climate and environmental work carries high emotional stakes, and staff can feel the weight of the mission in ways that accumulate into burnout." },
  ...
  { id: "advocacy", label: "Advocacy / Policy", icon: "📣", sectorMultiplier: 1.10,
    sectorNote: "Policy context and relationship networks are hard to transfer, adding a ~10% knowledge-loss premium to replacement costs.",
    ...
    risks: [
      ...
      { factor: "Policy cycle misalignment",      detail: "Election cycles and legislative sessions create natural pressure points where staff reassess their role, and departure often clusters around these moments." },
  ...
  { id: "other", label: "Other Nonprofit", icon: "◎", sectorMultiplier: 1.00,
    sectorNote: "Baseline nonprofit average applied with no sector-specific adjustment.",
...
const ORG_SIZES = [
  { id: "s1",  label: "1–9 staff",    employees: 5   },
  { id: "s2",  label: "10–19 staff",  employees: 14  },
  { id: "s3",  label: "20–29 staff",  employees: 24  },
  { id: "s4",  label: "30–39 staff",  employees: 34  },
  { id: "s5",  label: "40–49 staff",  employees: 44  },
  { id: "s6",  label: "50–59 staff",  employees: 54  },
  { id: "s7",  label: "60–69 staff",  employees: 64  },
  { id: "s8",  label: "70–79 staff",  employees: 74  },
  { id: "s9",  label: "80–89 staff",  employees: 84  },
  { id: "s10", label: "90–100 staff", employees: 95  },
];

const SALARY_RANGES = [
  { id: "entry",  label: "Entry-level & Frontline Staff", subtitle: "Program assistants, case workers, admin, volunteer coordinators", range: "$32,000 – $45,000", midpoint: 38000, complexity: "entry"  },
  { id: "mid",    label: "Mid-level & Licensed Professionals", subtitle: "Program managers, grant writers, licensed clinicians, HR, marketing", range: "$45,000 – $65,000", midpoint: 55000, complexity: "mid"    },
  { id: "senior", label: "Directors & Senior Management", subtitle: "Executive directors, finance directors, senior program directors", range: "$65,000 – $95,000", midpoint: 78000, complexity: "senior" },
];

const TURNOVER_RATES = [
  { id: "low",     label: "Below Average", subtitle: "Under 15% — strong retention, below the nonprofit norm", rate: 0.12 },
  { id: "typical", label: "Average",       subtitle: "15–25% — near the ~20% nonprofit sector average",       rate: 0.20 },
  { id: "high",    label: "Above Average", subtitle: "25–40% — elevated, common in human services and health", rate: 0.32 },
  { id: "crisis",  label: "Very High",     subtitle: "40%+ — significantly above sector norms",               rate: 0.50 },
];

const BASE_MULTIPLIERS = { entry: 0.65, mid: 1.00, senior: 1.50 };
const MULTIPLIER_LABELS = {
  entry:  "65% of salary — entry-level and frontline roles typically cost less to replace due to lower role complexity",
  mid:    "100% of salary — mid-level and licensed professionals require more specialized recruitment and longer ramp-up",
  senior: "150% of salary — director and senior management roles carry the highest recruitment and knowledge-transfer costs",
};
...
// ─── Calculation ──────────────────────────────────────────────────────────────
function calcCosts(orgType, size, salary, departures) {
  ...
  const totalRaw            = departures * salary.midpoint * effectiveMultiplier;
  const total               = Math.round(totalRaw / 1000) * 1000;

  const costPerDeparture    = departures > 0 ? Math.round(total / departures) : 0;
  ...
  const impliedRate         = Math.round((departures / employees) * 100);

  return { total, perMonth, departures, costPerDeparture, employees, baseMultiplier,
           sectorMultiplier, effectiveMultiplier, visibleTotal, invisibleTotal, impliedRate };
}

// ─── Animated counter ─────────────────────────────────────────────────────────
function AnimatedNumber({ value }) {
  const rounded = Math.round(value / 1000) * 1000;
  return <span>${rounded.toLocaleString()}</span>;
}

// ─── Cost row ─────────────────────────────────────────────────────────────────
function CostRow({ label, amount, example, barColor, totalPct, delay = 0 }) {
  ...
  return (
    <div style={{ marginBottom: "14px" }}>
      ...
      <div style={{ height: "20px", background: C.grayLight, borderRadius: "4px", position: "relative", overflow: "hidden" }}>
        <div style={{
          position: "absolute", top: 0, left: 0,
          height: "100%", width: filled ? `${totalPct * 100}%` : "0%",
          background: barColor, borderRadius: "4px",
          transition: `width 0.9s ease ${delay}ms`,
        }} />
        <span style={{
          position: "absolute", top: "50%", transform: "translateY(-50%)",
          left: filled ? `calc(${totalPct * 100}% + 6px)` : `calc(${totalPct * 100}% + 6px)`,
...
function MethRow({ label, value, detail, source }) {
  return (
    <div style={{ padding: "14px 0", borderBottom: `1px solid ${C.borderLight}` }}>
      <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: C.textFaint, marginBottom: "4px" }}>{label}</p>
      <p style={{ fontFamily: "'Lora', serif", fontSize: "15px", fontWeight: 700, color: C.navy, marginBottom: "5px" }}>{value}</p>
      <p style={{ fontFamily: "Inter, sans-serif", fontSize: "12px", color: C.textMuted, lineHeight: 1.65 }}>{detail}</p>
      {source && <p style={{ fontFamily: "Inter, sans-serif", fontSize: "10px", color: C.textFaint, marginTop: "4px", fontStyle: "italic" }}>Source: {source}</p>}
    </div>
  );
}

function MethodologyAccordion({ orgType, size, salary, departures, calc }) {
  const [open, setOpen] = useState(false);
  const { costPerDeparture, total, baseMultiplier, sectorMultiplier, effectiveMultiplier, impliedRate } = calc;
  ...
  return (
    <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderLeft: `4px solid ${C.navy}`, borderRadius: "12px", marginBottom: "16px", overflow: "hidden", boxShadow: C.shadow }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width: "100%", background: "transparent", border: "none", padding: "18px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", transition: "background 0.15s ease" }}
        onMouseEnter={e => e.currentTarget.style.background = C.navyLight}
        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
        <div style={{ textAlign: "left" }}>
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: C.textFaint, marginBottom: "6px" }}>Methodology</p>
          <p style={{ fontFamily: "'Lora', serif", fontSize: "18px", fontWeight: 700, color: C.navy, lineHeight: 1.3 }}>How we calculated your turnover cost</p>
        </div>
        <span style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform 0.25s", fontSize: "16px", color: C.navy, flexShrink: 0, marginLeft: "12px" }}>▾</span>
      </button>
      <div style={{ maxHeight: open ? "1500px" : 0, overflow: "hidden", transition: "max-height 0.45s ease" }}>
        <div style={{ padding: "0 20px 24px", borderTop: `1px solid ${C.border}` }}>
          <div style={{ background: C.navyLight, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "14px 16px", margin: "16px 0", fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMid, lineHeight: 2.1 }}>
            ...
            <div><strong style={{ color: C.navy }}>{departures} departure{departures !== 1 ? "s" : ""}</strong> × <span style={{ color: C.teal, fontWeight: 600 }}>${salary.midpoint.toLocaleString()} avg. salary</span></div>
            <div>
          ...
          <MethRow label="Staff count" value={size.label}
            detail={`${size.employees} full-time staff used in the calculation.`} />
          <MethRow label="Staff tier" value={salary.label}
            detail={`Midpoint salary of $${salary.midpoint.toLocaleString()} used. ${MULTIPLIER_LABELS[salary.complexity]}.`}
            source="Nonprofit Association of the Midlands 2024 Salary & Benefits Survey (Nebraska/Iowa); Charity Charge Nonprofit Salary Guide 2024; ZipRecruiter Nonprofit Salary Data 2026. National figures adjusted for Midwest cost of living." />
          <MethRow label="Departures" value={`${departures} departure${departures !== 1 ? "s" : ""}`}
            detail={`Implies a turnover rate of approximately ${impliedRate}% for your organization. The nonprofit sector averages approximately 20% annually, compared to 12% across all industries.`}
            source="Givebutter Nonprofit Burnout Report (Nov. 2025); RallyUp Nonprofit Turnover Report (Jul. 2025)" />
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: "10px", color: C.textFaint, marginTop: "14px", lineHeight: 1.6 }}>
            Sources: SHRM Human Capital Benchmarking Report; Boushey & Glynn, "There Are Significant Business Costs to Replacing Employees" (Center for American Progress); Givebutter Nonprofit Burnout Report (Nov. 2025); RallyUp Nonprofit Turnover Report (Jul. 2025); Nonprofit Association of the Midlands 2024 Salary & Benefits Survey; Charity Charge Nonprofit Salary Guide 2024; ZipRecruiter 2026.
          </p>
...
// ─── Staff slider ─────────────────────────────────────────────────────────────
function StaffSlider({ value, onChange }) {
  const n = value || 1;
  return (
    <div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "24px" }}>
        <span style={{ fontFamily: "'Lora', serif", fontSize: "64px", fontWeight: 700, color: C.navy, lineHeight: 1 }}>{n}</span>
        <span style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMuted, fontWeight: 500, marginTop: "6px", letterSpacing: "0.05em", textTransform: "uppercase" }}>staff</span>
      </div>
...
// ─── Departure grid ───────────────────────────────────────────────────────────
function DepartureGrid({ employees, selected, onChange }) {
  const avg = Math.max(1, Math.round(employees * 0.20));
  const rawOptions = [avg - 2, avg - 1, avg, avg + 1, avg + 2];
  const minVal = Math.min(...rawOptions);
  const shift = minVal < 1 ? 1 - minVal : 0;
  const options = rawOptions.map(n => ({
    id: String(n + shift),
    value: n + shift,
    isAverage: (n + shift) === avg,
  }));
  return (
...
// ─── Departure button ─────────────────────────────────────────────────────────
function DepartureButton({ opt, selected, onChange }) {
  const [hovered, setHovered] = useState(false);
  const [pressed, setPressed] = useState(false);
  const isAvg = opt.isAverage;

  const bg = selected
    ? (isAvg ? C.teal : C.navy)
    : isAvg
      ? C.tealLight
      : (hovered ? C.navyLight : C.cardGradAlt);
  const border = selected
    ? (isAvg ? C.teal : C.navy)
    : isAvg
      ? C.tealBorder
      : (hovered ? C.navy : C.border);
  const textColor = selected ? "#fff" : (isAvg ? C.tealDark : C.navy);
  const transform = pressed ? "scale(0.95) translateY(1px)" : hovered ? "scale(1.02) translateY(-1px)" : "none";
  const shadow = selected ? (isAvg ? C.shadowTeal : C.shadowNavy) : hovered ? C.shadow : C.shadowSm;
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: "12px", padding: "14px 8px 12px",
        cursor: "pointer", transition: "all 0.15s ease",
        transform, boxShadow: shadow,
        display: "flex", flexDirection: "column", alignItems: "center", gap: "6px",
      }}
    >
      <span style={{ fontFamily: "'Lora', serif", fontSize: "28px", fontWeight: 700, color: textColor, lineHeight: 1 }}>
        {opt.value}
      </span>
      {isAvg && (
        <span style={{
          ...
          color: selected ? "rgba(255,255,255,0.85)" : C.tealDark,
...
function OptionButton({ opt, selected, onChange }) {
  ...
  return (
    <button
      ...
      onMouseUp={() => setPressed(false)}
      style={{
        background:    selected        ? C.navyLight
                     : pressed        ? C.navyLight
                     : hovered        ? "#f0f4f8"
                     : C.cardGradAlt,
        border:        `1.5px solid ${selected ? C.navy : hovered ? C.navy : C.border}`,
        borderRadius:  "9px",
        padding:       "13px 10px",
        cursor:        "pointer",
        color:         selected        ? C.navy
                     : hovered        ? C.navy
                     : C.textMid,
        fontSize:      "13px",
        fontFamily:    "Inter, sans-serif",
        fontWeight:    selected || hovered ? 700 : 400,
        textAlign:     "center",
        lineHeight:    1.4,
        transform:     pressed          ? "scale(0.96) translateY(1px)"
                     ...
                     : "scale(1)",
        boxShadow:     selected        ? C.shadowNavy
                     : pressed        ? "none"
                     : hovered        ? C.shadow
                     : C.shadowSm,
    ...
    >
      <span style={{ display: "block", fontSize: "15px", fontWeight: selected || hovered ? 700 : 600 }}>{opt.label}</span>
      {opt.subtitle && <span style={{ display: "block", fontSize: "11px", fontWeight: 400, marginTop: "4px", opacity: 0.75, lineHeight: 1.4 }}>{opt.subtitle}</span>}
      {opt.range && <span style={{ display: "block", fontSize: "11px", fontWeight: 600, marginTop: "6px", opacity: 0.6 }}>Typical Midwest range: {opt.range}</span>}
    </button>
  );
}
...
// ─── Result screen ────────────────────────────────────────────────────────────
function ResultScreen({ orgType, size, salary, departures, onReset }) {
  const calc = calcCosts(orgType, size, salary, departures);
  const { total, perMonth, costPerDeparture, visibleTotal, invisibleTotal, impliedRate } = calc;
  ...
  const handleCopy = () => {
    const text = [
      ...
      `════════════════════════════════════════`,
      ``,
      `ESTIMATE: ${orgType.label.toUpperCase()}`,
      `Generated: ${timestamp}`,
      ``,
      `ESTIMATED ANNUAL TURNOVER COST`,
      `  $${total.toLocaleString()} / year`,
      `  $${perMonth.toLocaleString()} / month`,
      `  Equivalent to ${salaryMonths} months of a full staff salary`,
      ``,
      `MULTI-YEAR PROJECTION (at current rate)`,
      `  3 years:  $${threeYear.toLocaleString()}`,
      `  5 years:  $${fiveYear.toLocaleString()}`,
      ``,
      `INPUTS`,
      `  Organization type:  ${orgType.label}`,
      `  Staff size:         ${size.label}`,
      `  Role type:          ${salary.label}`,
      `  Departures/year:    ${departures} (implied rate: ~${impliedRate}%)`,
      ...
      ``,
      `RETENTION ROI`,
      `  Preventing one departure saves: $${costPerDeparture.toLocaleString()}`,
      ``,
      `────────────────────────────────────────`,
      `Estimates based on SHRM, Center for American Progress,`,
      `Nonprofit HR, and Nonprofit Association of the Midlands research.`,
      `This estimate is for planning purposes. Actual costs vary.`,
      ``,
  ...
  return (
    <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(16px)", transition: "all 0.5s ease" }}>
      ...
      {/* ── Input summary bar ── */}
      <div className="no-print" style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "6px", marginBottom: "24px" }}>
        {[
          orgType.label,
          `${size.employees} staff`,
          salary.label,
          `${departures} departure${departures !== 1 ? "s" : ""}/year`,
        ].map((crumb, i, arr) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            {i > 0 && <span style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textFaint }}>›</span>}
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", fontWeight: 600, color: C.teal }}>{crumb}</span>
          </div>
        ))}
        <span style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textFaint, marginLeft: "4px" }}>·</span>
        <button onClick={onReset} style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", fontWeight: 600, color: C.textFaint, background: "none", border: "none", cursor: "pointer", padding: "0", textDecoration: "underline", textDecorationColor: "transparent", transition: "color 0.15s, text-decoration-color 0.15s" }}
          onMouseEnter={e => { e.target.style.color = C.navy; e.target.style.textDecorationColor = C.navy; }}
          onMouseLeave={e => { e.target.style.color = C.textFaint; e.target.style.textDecorationColor = "transparent"; }}>
          Recalculate
        </button>
      </div>
        ...
        <div style={{ display: "flex", alignItems: "stretch", gap: "14px" }}>
          ...
          <div>
            <p style={{ fontFamily: "'Lora', serif", fontSize: "18px", fontWeight: 700, color: C.navy, lineHeight: 1.35, marginBottom: "8px" }}>
              That's the equivalent of <span style={{ color: C.navy, fontWeight: 700 }}>{salaryMonths} months of a full staff salary</span> walking out the door every year.
            </p>
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: C.shadowSm, overflow: "hidden", marginBottom: "12px" }}>
        <div style={{ padding: "20px 24px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${C.borderLight}`, background: C.cardGradAlt, gap: "16px" }}>
          <div style={{ display: "flex", alignItems: "stretch", gap: "14px" }}>
        ...
        <div style={{ padding: "18px 22px 20px" }}>
          {INVISIBLE_ITEMS.map((item, i) => (
            <CostRow key={item.label} label={item.label} amount={Math.round(total * item.pct)}
              ...
              totalPct={item.pct} delay={300 + i * 80} />
          ))}
        </div>
      </div>
      ...
      <div style={{ background: C.navyLight, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "16px 20px", marginBottom: "16px", boxShadow: C.shadowSm }}>
        <SectionLabel text="Why do these costs feel so high?" />
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMuted, lineHeight: 1.75 }}>
          No nonprofit writes a single check for ${total.toLocaleString()}. These costs arrive quietly — through unpaid overtime, programs running below capacity, and donor relationships that slowly fade. The <strong>${invisibleTotal.toLocaleString()}</strong> that no one tracks is still real. It just gets absorbed instead of budgeted, reducing mission impact without ever appearing on a balance sheet.
        </p>
      </div>

      {/* ── RETENTION ROI ── */}
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: C.shadow, padding: "20px 24px", marginBottom: "16px" }}>
        <SectionLabel text="Retention ROI" />
        <div style={{ display: "flex", alignItems: "flex-start", gap: "16px" }}>
          <div style={{ flexShrink: 0, width: "4px", alignSelf: "stretch", borderRadius: "2px", background: C.navy }} />
          {departures >= 3 ? (
            <div>
              <p style={{ fontFamily: "'Lora', serif", fontSize: "18px", fontWeight: 700, color: C.navy, lineHeight: 1.3, marginBottom: "6px" }}>
                Preventing even one departure saves <span style={{ color: C.teal }}>${costPerDeparture.toLocaleString()}</span>
              </p>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMid, lineHeight: 1.7 }}>
                At <strong>{departures} departures</strong> and <strong>${costPerDeparture.toLocaleString()} per departure</strong>, reducing that number by just one pays for meaningful retention investment. Any HR infrastructure that costs less than ${costPerDeparture.toLocaleString()} per person represents a direct net gain.
              </p>
            </div>
          ) : (
            <div>
              ...
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: "13px", color: C.textMid, lineHeight: 1.7 }}>
                At your current scale, each departure carries significant individual weight. Retention investments that cost less than ${costPerDeparture.toLocaleString()} per person pay for themselves immediately.
              </p>
            </div>
          )}
        </div>
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "12px", boxShadow: C.shadowSm, padding: "20px 24px", marginBottom: "16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "14px" }}>
          {[
            { label: "This year",   amount: total,    note: "Current estimate" },
            { label: "3-year total", amount: threeYear, note: "At current rate" },
            { label: "5-year total", amount: fiveYear,  note: "At current rate" },
          ].map((item, i) => (
            <div key={item.label} style={{ background: i === 0 ? C.navyLight : i === 1 ? "linear-gradient(160deg, #e6f7f7, #d0eeee)" : C.cardGradNavy, borderRadius: "8px", padding: "14px 16px" }}>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", fontWeight: 700, color: i === 2 ? "rgba(255,255,255,0.5)" : C.textFaint, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "6px" }}>{item.label}</p>
              <p style={{ fontFamily: "'Lora', serif", fontSize: i === 0 ? "20px" : i === 1 ? "22px" : "24px", fontWeight: 700, color: i === 2 ? C.teal : C.navy, lineHeight: 1 }}>${item.amount.toLocaleString()}</p>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: i === 2 ? "rgba(255,255,255,0.4)" : C.textFaint, marginTop: "4px" }}>{item.note}</p>
            </div>
          ))}
        </div>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "12px", color: C.textFaint, lineHeight: 1.7 }}>
          Projections assume consistent staffing levels and turnover rate. Real costs typically increase over time as salaries rise and roles become more specialized.
        </p>
      </div>
        ...
        <div style={{ padding: "18px 24px 20px", borderTop: `1px solid ${C.borderLight}`, background: C.navyLight }}>
          ...
          {orgType.firstSteps.map((step, i) => (
            <div key={i} style={{ display: "flex", gap: "10px", marginBottom: "10px", alignItems: "flex-start" }}>
              <div style={{ flexShrink: 0, width: "18px", height: "18px", background: C.navy, borderRadius: "3px", display: "flex", alignItems: "center", justifyContent: "center", marginTop: "2px" }}>
          ...
          <div style={{ marginTop: "18px", paddingTop: "16px", borderTop: `1px solid ${C.border}`, display: "flex", gap: "14px", alignItems: "stretch" }}>
            <div style={{ width: "3px", borderRadius: "2px", background: C.navy, flexShrink: 0 }} />
      ...
      {/* ── Methodology ── */}
      <MethodologyAccordion orgType={orgType} size={size} salary={salary} departures={departures} calc={calc} />
      ...
      <div style={{ background: C.cardGrad, border: `1px solid ${C.border}`, borderRadius: "14px", boxShadow: C.shadow, padding: "32px", textAlign: "center", marginBottom: "20px" }}>
        <p style={{ fontFamily: "'Lora', serif", fontSize: "22px", color: C.navy, marginBottom: "10px", lineHeight: 1.4 }}>You can't eliminate turnover. But you can build an organization where people want to stay.</p>
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "14px", color: C.textMuted, lineHeight: 1.8, maxWidth: "420px", margin: "0 auto" }}>
          You now have the number, the risk factors, and the first steps. Filament provides HR support across recruitment, compliance, performance management, and benefits administration: the building blocks that address the root causes and create conditions where great people want to stay.
        </p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px", paddingBottom: "24px" }}>
        ...
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint, marginTop: "4px" }}>Estimate generated {timestamp}</p>
      </div>
    </div>
  );
}
...
// ─── Steps ────────────────────────────────────────────────────────────────────
const STEPS = [
  { id: "orgType",    crumb: "Organization type", question: "What best describes your organization?",         sub: "Different missions, different pressures. Your sector helps us give you a more accurate number." },
  { id: "size",       crumb: "Staff size",         question: "How many full-time staff do you have?",           sub: "Drag to your number. Part-time and contract staff have different replacement costs, so we focus on full-time employees." },
  { id: "salary",     crumb: "Role type",          question: "Which type of role are you calculating the cost to replace?", sub: "You can run the calculator again for different role types." },
  { id: "departures", crumb: "Departures/year",    question: "How many departures per year do you want to calculate for?", sub: "The highlighted option reflects the nonprofit sector average for your staff size. You can run the calculator again to compare scenarios." },
];
const OPTIONS_MAP = {
  orgType:  { options: ORG_TYPES,      columns: 4 },
  salary:   { options: SALARY_RANGES,  columns: 1 },
};

// ─── App ──────────────────────────────────────────────────────────────────────
export default function App() {
  const [step,    setStep]    = useState(0);
  const [answers, setAnswers] = useState({ orgType: null, size: null, salary: null, departures: "" });
  const [done,    setDone]    = useState(false);
  const [animIn,  setAnimIn]  = useState(true);

  const stepKey       = STEPS[step]?.id;
  const currentAnswer = answers[stepKey];
  const canContinue   = stepKey === "departures"
                        ? !!answers.departures
                        : stepKey === "size"
                        ? !!answers.size
                        : !!currentAnswer;
  ...
  const goToStep = (i) => {
    go(() => {
      ...
      for (let j = i; j < STEPS.length; j++) {
        const key = STEPS[j].id;
        cleared[key] = key === "departures" ? "" : null;
      }
      setAnswers(prev => ({ ...prev, ...cleared }));
      setStep(i);
    });
  };
  ...
  return (
    <>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media print {
          ...
          * { box-shadow: none !important; }
          ...
          body::before {
            content: "FILAMENT ESSENTIAL SERVICES  ·  HR Turnover Cost Calculator  ·  filamentservices.org";
            display: block;
          ...
          body::after {
            content: "Estimates based on SHRM, Center for American Progress, Nonprofit HR, and Nonprofit Association of the Midlands research. For planning purposes only.  ·  filamentservices.org";
            display: block;
            font-family: Inter, sans-serif;
            font-size: 8px;
            color: #8a9ab0;
      ...
      <div style={{ minHeight: "100vh", background: "#ffffff", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 20px 80px" }}>
        ...
        {/* Breadcrumb */}
        {!done && (
          <div className="no-print" style={{ width: "100%", maxWidth: "660px", padding: "20px 0 0", marginBottom: "28px", display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
            {STEPS.map((s, i) => {
              const ans = answers[s.id];
              const isComplete = i < step;
              const isCurrent  = i === step;
              const isFuture   = i > step;
              let crumbLabel = s.crumb;
              if (isComplete && ans) {
                if (s.id === "size")       crumbLabel = `${ans.employees} staff`;
                else if (s.id === "departures") crumbLabel = `${ans} departure${ans !== 1 ? "s" : ""}`;
                else crumbLabel = ans.crumb || ans.label;
              }
              return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  ...
                  <button
                    onClick={() => isComplete && goToStep(i)}
                    style={{
                      fontFamily: "Inter, sans-serif",
                      fontSize: "13px",
                      fontWeight: isCurrent ? 700 : isComplete ? 600 : 400,
                      color: isCurrent ? C.navy : isComplete ? C.teal : C.textFaint,
                      background: "none",
                      border: "none",
                      padding: "2px 0",
                    ...
                    onMouseEnter={e => { if (isComplete) e.target.style.textDecorationColor = C.teal; }}
                    onMouseLeave={e => { if (isComplete) e.target.style.textDecorationColor = "transparent"; }}
                  >
                    {crumbLabel}
                  </button>
                </div>
        ...
        <div style={{ width: "100%", maxWidth: "660px", opacity: animIn ? 1 : 0, transform: animIn ? "translateY(0)" : "translateY(10px)", transition: "all 0.22s ease" }}>
          {done ? (
            <ResultScreen
              ...
              salary={answers.salary}  departures={Number(answers.departures)}
              onReset={() => go(() => { setDone(false); setStep(0); setAnswers({ orgType: null, size: null, salary: null, departures: "" }); })}
            />
          ) : (
            <div style={{ background: C.cardGrad, borderRadius: "16px", boxShadow: C.shadowLg, overflow: "hidden" }}>
              ...
              <div style={{ padding: "36px 32px 32px" }}>
                ...
                {stepKey === "departures" ? (
                  <>
                    <div style={{ background: C.navyLight, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "10px 14px", marginBottom: "20px", display: "inline-flex", alignItems: "center", gap: "8px" }}>
                      <span style={{ fontFamily: "Inter, sans-serif", fontSize: "12px", color: C.textFaint, fontWeight: 500 }}>Your staff count:</span>
                      <span style={{ fontFamily: "'Lora', serif", fontSize: "16px", fontWeight: 700, color: C.navy }}>{answers.size?.employees || 10} staff</span>
                    </div>
                    <DepartureGrid
                      employees={answers.size?.employees || 10}
                      selected={answers.departures}
                      onChange={v => setAnswers(prev => ({ ...prev, departures: v }))}
                    />
                    <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint, marginTop: "14px", lineHeight: 1.6, fontStyle: "italic" }}>
                      Sector average based on a ~20% annual turnover rate for nonprofits. Source: Givebutter Nonprofit Burnout Report (Nov. 2025); RallyUp Nonprofit Turnover Report (Jul. 2025).
                    </p>
                  </>
                ) : stepKey === "size" ? (
                  <StaffSlider
                    value={answers.size?.employees}
                    onChange={n => setAnswers(prev => ({ ...prev, size: { employees: n, label: `${n} staff` } }))}
                  />
                ) : (
                <OptionGrid
                ...
                {step > 0 && (
                  <div style={{ textAlign: "center", marginTop: "14px" }}>
                    <PressButton variant="ghost" onClick={() => go(() => {
                      setAnswers(prev => ({ ...prev, [stepKey]: (stepKey === "departures" || stepKey === "size") ? null : null }));
                      setStep(s => s - 1);
                    })}
        ...
        <p className="no-print" style={{ marginTop: "28px", fontFamily: "Inter, sans-serif", fontSize: "10px", color: C.textFaint, textAlign: "center", letterSpacing: "0.04em" }}>
          Estimates based on SHRM, Center for American Progress, Nonprofit HR, and Nonprofit Association of the Midlands research
        </p>
      </div>
    </>
  );
