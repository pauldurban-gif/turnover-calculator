...
const ORG_SIZES = [
  { id: "s1",  label: "1–10 staff",    employees: 5  },
  { id: "s2",  label: "11–20 staff",   employees: 15 },
  { id: "s3",  label: "21–30 staff",   employees: 25 },
  { id: "s4",  label: "31–40 staff",   employees: 35 },
  { id: "s5",  label: "41–50 staff",   employees: 45 },
  { id: "s6",  label: "51–60 staff",   employees: 55 },
  { id: "s7",  label: "61–70 staff",   employees: 65 },
  { id: "s8",  label: "71–80 staff",   employees: 75 },
  { id: "s9",  label: "81–90 staff",   employees: 85 },
  { id: "s10", label: "91–100 staff",  employees: 95 },
];
      ...
      <div style={{ maxHeight: open ? "1500px" : 0, overflow: "hidden", transition: "max-height 0.45s ease" }}>
        <div style={{ padding: "0 20px 24px", borderTop: `1px solid ${C.border}` }}>
          ...
          <MethRow label="Departures" value={`${departures} departure${departures !== 1 ? "s" : ""}`}
            ...
            source="Nonprofit HR 2023 State of the Nonprofit Sector; RallyUp Nonprofit Turnover Report 2024; Cerini & Associates 2024 Nonprofit Trends" />
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: "10px", color: C.textFaint, marginTop: "14px", lineHeight: 1.6 }}>
            Sources: SHRM Human Capital Benchmarking Report; Boushey & Glynn, "There Are Significant Business Costs to Replacing Employees" (Center for American Progress); Nonprofit HR Annual Surveys 2021–2023; Nonprofit Association of the Midlands 2024 Salary & Benefits Survey; Charity Charge Nonprofit Salary Guide 2024; ZipRecruiter 2026.
          </p>
...
function OptionButton({ opt, selected, onChange }) {
  ...
  return (
    ...
    >
      <span style={{ display: "block", fontWeight: selected || hovered ? 700 : 500 }}>{opt.label}</span>
...
function ResultScreen({ orgType, size, salary, departures, onReset }) {
  ...
  const handleCopy = () => {
    const text = [
      ...
      `  • Departures calculated: ${departures} at $${costPerDeparture.toLocaleString()} each (implied rate: ~${impliedRate}%)`,
      ``,
      `COST BREAKDOWN:`,
    ...
    <div style={{ opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(16px)", transition: "all 0.5s ease" }}>
      ...
      <div style={{ background: C.navyLight, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "16px 20px", marginBottom: "16px", boxShadow: C.shadowSm }}>
        <p style={{ fontFamily: "'Lora', serif", fontSize: "14px", fontStyle: "italic", color: C.navy, lineHeight: 1.75, textAlign: "center" }}>
          Most leaders have only ever mentally budgeted <strong style={{ fontStyle: "normal" }}>${visibleTotal.toLocaleString()}</strong>. The other <strong style={{ fontStyle: "normal", color: C.teal }}>${invisibleTotal.toLocaleString()}</strong> is just as real, scattered across timesheets, morale, and muscle memory that no one ever measured.
        </p>
      </div>
...
// ─── Steps ────────────────────────────────────────────────────────────────────
const STEPS = [
  { id: "orgType",    question: "What best describes your organization?",         sub: "Different missions, different pressures. Your sector helps us give you a more accurate number." },
  { id: "size",       question: "How many full-time staff do you have?",           sub: "Part-time and contract staff have different replacement costs, so we focus on full-time employees for a cleaner estimate." },
  { id: "salary",     question: "Which type of role are you calculating the cost to replace?", sub: "You can run the calculator again for different role types." },
  { id: "departures", question: "How many departures per year do you want to calculate for?", sub: "The highlighted option reflects the nonprofit sector average for your staff size. You can run the calculator again to compare scenarios." },
];
const OPTIONS_MAP = {
  orgType:  { options: ORG_TYPES,      columns: 4 },
  size:     { options: ORG_SIZES,      columns: 5 },
              ...
              <div style={{ padding: "36px 32px 32px" }}>
                ...
                {stepKey === "departures" ? (
                  ...
                  <p style={{ fontFamily: "Inter, sans-serif", fontSize: "11px", color: C.textFaint, marginTop: "14px", lineHeight: 1.6, fontStyle: "italic" }}>
                    Sector average based on a ~20% annual turnover rate for Midwest nonprofits. Source: Nonprofit HR 2023 State of the Nonprofit Sector; Cerini & Associates 2024 Nonprofit Trends.
                  </p>
                ) : (
                <OptionGrid
                ...
                {step > 0 && (
                  <div style={{ textAlign: "center", marginTop: "14px" }}>
                    <PressButton variant="ghost" onClick={() => go(() => {
                      setAnswers(prev => ({ ...prev, [stepKey]: stepKey === "departures" ? "" : null }));
                      setStep(s => s - 1);
                    })}
